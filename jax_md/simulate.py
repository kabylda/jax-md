...
def npt_nose_hoover(energy_fn: Callable[..., Array],
  ...
  def init_fn(key, R, box, mass=f32(1.0), **kwargs):
    ...
    state = NPTNoseHooverState(
      R, None, jnp.asarray(force_fn(jnp.asarray(R, jnp.float32), box=jnp.asarray(box, jnp.float32), **kwargs), R.dtype),
  ...
  def box_force(alpha, vol, box_fn, position, momentum, mass, force, pressure,
    ...
    def U(eps):
      return energy_fn(jnp.asarray(position, jnp.float32), box=jnp.asarray(box_fn(vol), jnp.float32), perturbation=(1 + eps),
                       **kwargs)

    dUdV = grad(U)
    KE2 = util.high_precision_sum(momentum ** 2 / mass)

    return alpha * KE2 - dUdV(jnp.asarray(0.0, jnp.float32)) - pressure * vol * dim
